...
import (
	...
	"github.com/richardcase/herald/internal/ui/components/footer"
	"github.com/richardcase/herald/internal/ui/components/section"
	"github.com/richardcase/herald/internal/ui/components/tabs"
	"github.com/richardcase/herald/internal/ui/constants"
	"github.com/richardcase/herald/internal/ui/context"
	"github.com/richardcase/herald/internal/ui/keys"
...
func NewModel(configPath string) Model {
	...
	m.footer = footer
	//m.notificationSidebar = notificiationsidebar.NewModel(m.ctx)
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var (
		cmd tea.Cmd
		//sidebarCmd   tea.Cmd
		//notificationSidebarCmd tea.Cmd
		footerCmd   tea.Cmd
		cmds        []tea.Cmd
	...
	case tea.KeyMsg:
		...
		// if m.notificationSidebar.IsTextInputBoxFocused() {
		// 	m.notificationSidebar, cmd = m.notificationSidebar.Update(msg)
		// 	m.syncSidebar()
		// 	return m, cmd
		// }
		...
		case key.Matches(msg, m.keys.OpenGithub): //TODO
		...
		// case key.Matches(msg, keys.NotificationKeys.Comment):
		// 	m.sidebar.IsOpen = true
		// 	if m.ctx.View == config.NotificationsView {
		// 		cmd = m.notificationSidebar.SetIsCommenting(true)
		// 	}
		// 	m.syncMainContentWidth()
		// 	m.syncSidebar()
		...
		case key.Matches(msg, keys.NotificationKeys.Read, keys.NotificationKeys.Unread):

			var action string
			switch {
			case key.Matches(msg, keys.NotificationKeys.Read):
				action = "read"

			case key.Matches(msg, keys.NotificationKeys.Unread):
				action = "unread"
			}
