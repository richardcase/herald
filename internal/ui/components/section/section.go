...
import (
	...
	"github.com/richardcase/herald/internal/ui/components/prompt"
	"github.com/richardcase/herald/internal/ui/components/search"
...
func (m *Model) GetPromptConfirmation() string {
	if m.IsPromptConfirmationShown {
		var prompt string
		switch {
		case m.PromptConfirmationAction == "read" && m.Ctx.View == config.NotificationsView:
			prompt = "Are you sure you want to mark this notification as read? (Y/n) "
		}
