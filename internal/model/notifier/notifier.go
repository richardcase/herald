...
import (
	...
	"github.com/richardcase/herald/internal/model/tab/feed"
	"github.com/richardcase/herald/internal/model/tab/welcome"
)
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tab.NewTabMessage:
		...
		m.createNewTab(msg.Title, tab.Notifications)
		m.message = ""
...
func (m *Model) createNewTab(title string, tabType tab.Type) {
	...
	case tab.Notifications:
		//TODO: handle !ok
		_, filter := m.config.Filters.GetFilterByName(title)
		newTab = feed.New(
			m.config.Colors,
			m.windowWidth,
			m.windowHeight-5,
			title,
			filter.RepoFilter,
			m.config.Backend.FetchNotifications,
		)
	}
...
//	return m, m.config.Backend.FetchNotifications()
//}
...
func (m *Model) fetchFilters() tea.Cmd {
	return func() tea.Msg {
		items := make([]list.Item, len(m.config.Filters))
		for i, filter := range m.config.Filters {
			items[i] = simplelist.NewItem(filter.Name, filter.Description, "")
		}
